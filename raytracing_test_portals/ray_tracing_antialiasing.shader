...
side note: enable FXAA under ProjectSettings/Rendering/Quailty/Filters/Use_fxaa for anti ailasing
...
void CreateCameraRay(vec2 time,vec2 vps, vec2 coord,inout vec3 ray_origin,inout vec3 ray_direction,inout vec3 ray_energy)
{

	//vec2 uv = (coord * 2.0 - vps)/(vps.y);
	vec2 uv = (coord * 2.0 - vps)/(vps.y);
	//vec2 uv = vec2(coord+PixelOffset)/vps*2f-1f;
...
mat3 GetTangentSpace(vec3 normal)
{
    ...
    vec3 helper = vec3(1f, 0f, 0f);
    if (abs(normal.x) > 0.99f)
        helper = vec3(0f, 0f, 1f);
    // Generate vectors
...
vec3 SampleHemisphere(vec2 my_uv,vec3 normal)
{
    // Uniformly sample hemisphere direction
    float cosTheta = rand(my_uv);
    float sinTheta = sqrt(max(0.0f, 1.0f - cosTheta * cosTheta));
    float phi = 2f * PI * rand(my_uv);
...
{
    return clamp(dot(x, y) * f,0f,1f);
}

uniform bool lambert_diffuse = false;
// here's where everything on the screen is coloured in

vec3 Shade(vec2 my_uv,inout vec3 ray_origin,inout vec3 ray_direction,inout vec3 ray_energy, vec3 hit_position,float hit_distance,vec3 hit_normal,vec3 hit_albedo,vec3 hit_specular)
{
    ...
    if (hit_distance < 9900.0)// basically when a ray hits something
    {
		...
		else if(lambert_diffuse){
			// Diffuse shading
			ray_origin = hit_position + hit_normal * 0.001f;
			ray_direction = SampleHemisphere(my_uv,hit_normal);
			ray_energy *= 2f * hit_albedo * sdot(hit_normal, ray_direction,1f);
			return vec3(0.0f);
		}
...
void fragment() {
	if(active){
		...
		CreateCameraRay(vec2(TIME,0f),VIEWPORT_SIZE,FRAGCOORD.xy,ray_origin,ray_direction,ray_energy);
	    ...
	    for (int i = 0; i < BOUNCE; i++)
	    {
			...
			result += m_ray_energy * Shade(UV,ray_origin,ray_direction,ray_energy, hit_position,hit_distance,hit_normal,hit_albedo,hit_specular);
/*
